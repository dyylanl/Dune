...
void RecvThread::crearUnidad() {
    ...
    if (unitType == 'T') {
        std::cout << "crear tank " << unitId << ": (" << (uint16_t)pos[0] << "," << (uint16_t)pos[1] << ")" <<std::endl;
        Player player1("missileTank", m_textureManager, SDL2pp::Point((uint16_t)pos[0] * 30, (uint16_t)pos[1] * 30), SDL2pp::Point(34, 34));
        m_gameObjects.push_back(player1);
    }
    if (unitType == 'o') {
        std::cout << "crear otro: " << unitId << ":" << pos[0] << "," << pos[1] << std::endl;
        Player player1("carryall", m_textureManager, SDL2pp::Point((uint16_t)pos[0] * 30, (uint16_t)pos[1] * 30), SDL2pp::Point(64, 61));
        m_gameObjects.push_back(player1);
    }
}
